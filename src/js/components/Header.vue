<template>
  <header class="rs-header bg-theme py-3 sh-2 pos-sti pos-l-0 pos-r-0 pos-t-0 zi-10">
    <div class="container d-f ai-c jc-sb px-5 px-7:md">
      <div class="rs-logo">
        <routre-link to="/" class="d-ib">
          <svg-icon add-class="wd-sz-5 he-sz-5" href="/" name="rs-logo"></svg-icon>
        </routre-link>
      </div>
      <div class="d-f ai-c">
        <svg-icon add-class="wd-sz-5 he-sz-5" name="rs-github" target="_blank" href="https://github.com/moodrets/rest-css-framework"></svg-icon>
      </div>
    </div>
  </header>
</template>

<script>
  export default {

  }
</script>
